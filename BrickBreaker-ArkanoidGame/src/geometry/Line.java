...
public class Line {
    ...
    public Point closestIntersectionToStartOfLine(Rectangle rect) {
        ...
        List<Point> listPoint = rect.intersectionPoints(this);
        Point intersectionPoint =  null;
        double distance = this.length();
        // check which point from the list is the closest to the start of line.
        for (int i = 0; i < listPoint.size(); i++) {
            if (distance >= this.start.distance(listPoint.get(i))) {
                distance = this.start.distance(listPoint.get(i));
                intersectionPoint = listPoint.get(i);
            }
        }
     ...
     * @return - "true" if it is on the line, else return "false".
     */
    public boolean isPointOnLine(Point p) {
        if (Math.abs(p.distance(this.start) + p.distance(this.end) - this.length()) < 0.0001) {
            return true;
        }
        return false;
    }
